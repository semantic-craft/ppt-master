...
- **Content-faithful PPT beautification / re-layout (next direction)** — a new route that mirrors `template-fill-pptx`: where `template-fill` reuses a deck's design and swaps in new content, this one inverts it — keep a deck's content, redo its layout. Given an existing PPTX, **every piece of text is preserved verbatim (nothing added / removed / reworded)**; beautification happens only in layout, hierarchy, whitespace, pagination, and visual structure (an ugly deck is usually ugly in its density and hierarchy, not in its colors or fonts). It also **extracts and inherits the source deck's visual identity (palette / fonts / layout idiom)** so the newly generated native elements match the original — they can be copied straight back into the original PPT and still look coherent. Because of that, this route does *not* freely apply the `visual-style` catalog; it carries over the style lifted from the source deck (the existing [`create-brand`](../skills/ppt-master/workflows/create-brand.md) / `pptx_to_svg` theme extraction already does this). Technically it is still "generate a native, editable PPTX from scratch" (`ppt_to_md` extracts content → main pipeline → a brand-new deck), not a patch over the original file, so it stays clear of the #53 "text-only fill of an arbitrary template" line under Non-goals. The contract: **text untouched; visual identity taken from the source deck, not freely reskinned; all the freedom to beautify goes to layout and hierarchy.** Likely shape: a standalone workflow plus a content-faithful Strategist sub-mode. Open boundaries still to settle: strict pagination fidelity vs. allowing re-pagination, and how to handle tables / charts / existing images
- Otherwise: the mode / visual-style validation and calibration is settled (see "2026-06" above) — the structure (5 modes + 18 visual-styles + custom) is locked, the four adjacent pairs are consolidated into one Close-calls table, and the four calibration tightenings have landed. Apart from the direction above, future direction is driven by real usage signal and feedback; long-running improvements are under "Ongoing maintenance" below, and evaluated-out directions under "Non-goals"
