...
- **visual-style catalog grows to 14, aligned with image-renderings** — first four distilled from the [examples library](../examples/) (`brutalist` / `blueprint` / `memphis` / `zine`), then six more filling in the hand-drawn / textured renderings that have a layout twin: `sketch-notes` (warm paper + soft pastel blocks) · `ink-notes` (white field + black ink + sparse semantic accent) · `chalkboard` (dark slate + chalk) · `paper-cut` (layered cut-paper + soft inter-layer shadow) · `vintage-poster` (mid-century flat blocks + halftone) · `pixel-art` (pixel grid). Each style pairs with the same-named / same-temperament rendering, and the catalog is grouped into four families (structured-graphic / expressive-print / hand-drawn-educational / specialty). Photographic / painterly renderings (corporate-photo / nature / warm-scene / fantasy-animation / watercolor) deliberately get no layout twin — they describe how an *image* looks, not a deck layout, so they pair with a layout style instead. Zero HEX, Reference strength throughout
