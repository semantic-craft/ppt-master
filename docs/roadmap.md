...
- **visual-style catalog grows to 16, aligned with image-renderings + examples reclaimed** — first four distilled from the [examples library](../examples/) (`brutalist` / `blueprint` / `memphis` / `zine`), then six filling in the hand-drawn / textured renderings that have a layout twin (`sketch-notes` / `ink-notes` / `chalkboard` / `paper-cut` / `vintage-poster` / `pixel-art`), then two more reclaimed from still-uncovered example aesthetics: `ink-wash` (rice-paper whitespace + seal accent, from 藏拙 / 李子柒) · `glassmorphism` (dark frosted-glass panels + gradient light, from glassmorphism_demo, split out of soft-rounded). Each style pairs with the same-named / same-temperament rendering, and the catalog is grouped into four families (structured-graphic / expressive-print / hand-drawn-brush / specialty). Photographic / painterly renderings (corporate-photo / nature / warm-scene / fantasy-animation / watercolor) deliberately get no layout twin — they describe how an *image* looks, not a deck layout, so they pair with a layout style instead. Zero HEX, Reference strength throughout
