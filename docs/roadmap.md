...
- **Real-deck calibration of the illustration-slice pipeline (just landed)** — the sheet → slice → place chain and the `illustration_usage` toggle shipped today (see "2026-06" above); next is calibration from real usage: whether one sheet sliced into many cells holds style / palette consistency in real generation, how robust `--trim` / `--alpha` are against irregular AI composition inside a cell, the offline readiness-gate manual-place + re-slice experience, and whether the Strategist turns `illustration_usage: use` into the right sparse-vs-recurring intensity. No mechanical threshold / quota added pre-emptively
