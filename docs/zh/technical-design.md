...
输出：
    ...
    # 显式 --svg-snapshot 才生成（用于分发 / 冻结快照存档）
    backup/<timestamp>/
    ├── presentation_svg.pptx                  ← SVG 快照版 pptx — 像素级视觉参考
...
| `backup/<ts>/<name>_svg.pptx`（preview，需 `--svg-snapshot` 显式开启） | 跨平台单文件分发、整体多页浏览、邮件附件 | 自包含、多页、PowerPoint / Keynote / WPS / LibreOffice 都能直接打开；`svg_final/` 是文件夹，分发不便。默认关闭——live preview 已经覆盖 dev / 诊断场景的 SVG 视觉参考需求 |
