...
- **源文档转换保真度提升一批** — 源材料进管线时更少丢信息：`doc_to_md` 把 Word 里的 OMML / Office Math 公式转成内联 LaTeX、`pdf_to_md` 识别 `Figure N |` 竖线分隔的图注、`ppt_to_md` 保留源 deck 已有的超链接（run 级外链 `[text](url)` / slide 内部跳转 `[text](#slide-N)` / shape 级点击，含危险 scheme 过滤与锚文本 Markdown 转义）。图注识别基于 [@suay1113](https://github.com/suay1113) [PR #191](https://github.com/hugohe3/ppt-master/pull/191)，超链接保留提炼自 [@ZhaoZuohong](https://github.com/ZhaoZuohong) [PR #155](https://github.com/hugohe3/ppt-master/pull/155)
...
- **内容保真的 PPT 美化 / 重排版（下一步方向）** — 一条与 `template-fill-pptx` 互为镜像的新路线：`template-fill` 是「复用某份 deck 的设计、换上新内容」，这条反过来——「保留某份 deck 的内容、重做它的版面」。输入一份现成 PPTX，**逐字保留全部文字内容（不增 / 不删 / 不改写）**，美化只发生在版面、层级、留白、分页与视觉结构上（丑 deck 的丑通常在密度与层级，不在它的色与字）。同时**从原 deck 提取并继承其视觉身份（配色 / 字体 / 版面语汇）**，让新生成的原生元素与原 deck 风格一致——可以直接复制回原 PPT 模板、看上去仍然协调；也因此这条路线不自由套用 `visual-style` catalog，而是沿用从源 deck 抽取的风格（现有 [`create-brand`](../../skills/ppt-master/workflows/create-brand.md) / `pptx_to_svg` 主题提取已能做）。技术上仍是「从零生成原生可编辑 PPTX」（`ppt_to_md` 抽内容 → 主管线 → 全新 deck），不是在原文件上打补丁，因此不碰 Non-goals 里 #53 那条「读任意模板只填字」的红线。关键契约：**文字一字不动；视觉身份取自原 deck，不自由换皮；美化的自由度全部留给版面与层级**。落地形态预计为一条独立 workflow + Strategist 的内容保真子模式。仍待定的边界：严格分页保真 vs 允许重排分页、表格 / 图表 / 已有配图如何处理
- 其余：mode / visual-style 体系的验证与校准已收口（见上「2026-06」），结构（5 mode + 18 visual-style + custom）定型、四对近邻消歧并成一张 Close-calls 表、四项校准收紧已落地。除上面这条外，后续方向由真实使用信号与反馈驱动；长期改进见下「持续维护方向」，已评估不做的见「明确不做」
