...
- **visual-style catalog 扩充至 16 个，与 image-renderings 对齐 + 示例库回收** — 先从[示例库](../../examples/)提炼 4 个（`brutalist` / `blueprint` / `memphis` / `zine`），再补齐 [`image-renderings`](../../skills/ppt-master/references/image-renderings/) 里有排版对应物的手绘 / 纹理风格 6 个（`sketch-notes` / `ink-notes` / `chalkboard` / `paper-cut` / `vintage-poster` / `pixel-art`），最后回收示例库里仍未覆盖的两种独立气质：`ink-wash`（新中式水墨留白 + 印章红，源自 藏拙 / 李子柒）· `glassmorphism`（深底磨砂玻璃面板 + 流光渐变，源自 glassmorphism_demo，从 soft-rounded 独立出来）。每个 style 与同名 / 同气质 rendering 配对，catalog 按 结构图形 / 表现印刷 / 手绘笔触 / 特殊 四组归类。摄影 / 绘画类 rendering（corporate-photo / nature / warm-scene / fantasy-animation / watercolor）刻意不建 layout 孪生——它们描述「图」的样子而非版面，配对某个 layout style 即可。全程零 HEX、Reference 强度
