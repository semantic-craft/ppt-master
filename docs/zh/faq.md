...
可以。导出的 PPTX 同时支持**页间转场**和**页内元素入场动画**，都通过 `svg_to_pptx.py` 的 CLI 参数控制。

**页间转场**（默认开启 0.4s `fade`）

```bash
# 换效果：fade / push / wipe / split / strips / cover / random
...
**页内动画**（默认关闭 —— 老用户更新后行为不变）

进入新页 → 单击一次 → 各语义组按 z-order 依次入场。开启方式：
