...
| **2 — realization** (re-derived from Tier 1) | page count · color · typography (font + size) · icons · formula policy · image usage + illustration usage + illustration strategy + generated-image style · generation mode · refine-spec toggle | the confirmed Tier 1 |
...
| `use` | Make illustrations part of the deck; the confirmed `illustration_strategy` controls how they are used |

**Strategy and source are separate.** `illustration_strategy` controls role and intensity; source always comes from `image_usage` (which maps to §VIII `Acquire Via`): `image_usage: none` always wins — write no decorative illustration rows even if `illustration_usage` is `use`. Otherwise, when `use` is set: an AI source may use the `ai` Illustration Sheet + `slice` workflow for ≥3 same-family spot illustrations; `provided` makes them `Type: Illustration` + `Acquire Via: user` rows; `web` makes them `web` rows only if licensing is acceptable; a deferred/copyright-sensitive plan makes them `placeholder` rows. The user chooses what role illustrations play, but never has to choose or see the internal sheet/slice implementation.

**Illustration strategy is user-facing and multi-select when illustrations are used.** Confirm UI writes `result.json.illustration_strategy` as an array containing one or more of `hero-only` / `sparse-spots` / `recurring-motif` / `image-as-canvas`; honor it exactly. If chat fallback or a legacy result omits the array while `illustration_usage: use`, choose a conservative default from content + `visual_style` and record that it was inferred. Strategist writes `design_spec.md §VIII Illustration System` with the selected strategies, a motif family, reuse rule, do-not-use pages, and boundary against icons / charts / photos. If the selected strategies include `sparse-spots` or `recurring-motif` and the plan needs ≥3 same-family small illustrations, use the `ai` Illustration Sheet + `slice` workflow by default; do not generate one AI image per spot.
