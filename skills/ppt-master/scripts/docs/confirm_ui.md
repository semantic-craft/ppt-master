...
- Binds `127.0.0.1:5050` by default and auto-opens the browser (suppress with `--no-browser`).
...
- `image_ai_path` and `image_strategy` are omitted from `result.json` unless `image_usage` is `ai` or a custom image plan that may include generated images. Both are honored downstream as confirmed choices: `image_ai_path` drives the Step 5 generation path (`image-generator.md` §7 — `host-native` forces the host tool even when `IMAGE_BACKEND` is set), and the chosen `image_strategy` candidate is locked verbatim by Strategist h.5 (no re-pick).
