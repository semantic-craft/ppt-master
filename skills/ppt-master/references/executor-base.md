...
**Font size is skin, not geometry (non-mirror).** A chart / layout template's hardcoded `font-size` values (often 11–16px, sized for the template's own dense placeholder text) are NOT inherited — classify each text into its `spec_lock.md` role and use that role's locked size, exactly as you re-skin color. **Structural roles (page title / body / subtitle / annotation / footnote) hold their one deck-wide size on every page** — the template's placeholder px never overrides it; same-role text drifting page to page is what makes a deck look unprofessional. **Geometry adapts to the type, never the reverse**: when the locked size is larger than the template's placeholder text, widen / heighten the card, open up spacing, recompute child `y` / `dy`, or drop an item. Do not shrink the font to fit the inherited container; if a page still overflows, thin the content upstream (fewer items / split the page). Only when a single block genuinely cannot fit after all of that may you shrink **that block** as a last resort — typically **≤2pt**, never a deck-wide structural role. (Mirror templates are the exception: §1.1 preserves their sizes verbatim — there the source deck's typography *is* the spec.)
...
- Font sizes follow a **ramp anchored on `typography.body`**, not a closed menu. **Structural roles — page title, body, subtitle, annotation / caption, footnote / page number — render at one consistent size deck-wide, taken from their `spec_lock` slot; never re-pick a structural role's size page by page or carry a template's placeholder px.** In-band intermediate sizes are for **special / feature elements only** (hero number, cover / section display title, a one-off emphasis figure); if such a size recurs, declare it as its own `spec_lock` slot so it too stays consistent. Sizes outside every band require extending the lock first.
