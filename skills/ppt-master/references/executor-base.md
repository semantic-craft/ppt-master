...
**Font size is skin, not geometry (non-mirror).** A chart / layout template's hardcoded `font-size` values (often 11–16px, sized for the template's own dense placeholder text) are NOT inherited — re-derive every text size from the `spec_lock.md` role sizes (`body` / `title` / `subtitle` / `annotation` / any role-specific slot), exactly as you re-skin color. The spec is the authority: text renders at its spec_lock role size, and **geometry adapts to the type, never the reverse.** When the spec_lock size is larger than the template's placeholder text, widen / heighten the card, open up spacing, recompute child `y` / `dy`, or drop an item to keep the page breathing — do not shrink the font to fit the inherited container. Only when a block genuinely cannot fit *after* the geometry has been reflowed may you shrink **that block** slightly — typically **≤2pt** below its spec_lock role size, never deck-wide and never as the first move. (Mirror templates are the exception: §1.1 preserves their sizes verbatim — there the source deck's typography *is* the spec.)
