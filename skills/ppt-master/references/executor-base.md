...
- Native chart typography mirrors the SVG fallback. Copy the fallback's visible chart text sizes into the matching metadata fields (`title_font_size`, `axis_font_size`, `note_font_size`, etc.); use `axis_title_font_size`, `legend_font_size`, or companion per-entry `font_size` only when the fallback visibly uses a separate size. If a fallback role has no explicit size, use the compact three-tier chart defaults from [shared-standards.md](shared-standards.md).
